...
//    其他数据类型通过归档方式转化为data
    ...
    aStudent.name  = @"男神";
    aStudent.number = @"628658562856";
    aStudent.sex = @"人妖";
    
    Student  *bStudent = [[Student alloc]init];
    bStudent.name  = @"女神";
    bStudent.number = @"12334145";
    bStudent.sex = @"妖";
    // 其他数据类型通过归档方式转化为data
    
    NSArray *arrayStudent = [NSArray arrayWithObjects:aStudent,bStudent, nil];
    [[NSUserDefaults standardUserDefaults]setObject:[NSKeyedArchiver archivedDataWithRootObject:arrayStudent] forKey:@"wuseya"];
    [[NSUserDefaults standardUserDefaults]synchronize];
    
    NSData *studentData = [[NSUserDefaults standardUserDefaults]objectForKey:@"wuseya"];
    NSArray *getArrayStudent = [NSKeyedUnarchiver unarchiveObjectWithData:studentData];
    for (Student *getStudent in getArrayStudent) {
